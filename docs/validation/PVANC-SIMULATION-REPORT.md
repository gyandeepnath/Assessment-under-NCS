...
3. **Random guesser** — *Expected:* no false reassurance, ideally an explicit validity flag. *Output:* the floor/completeness gates send the large majority to **inconclusive → retake**; **no run** reports within-expected at usable quality; and a **first-class validity detector** now explicitly flags guessing (`invalid-response-pattern`) and forces inconclusive. *Failure:* confident normal acuity — not seen. **Verdict: PASS; W5 addressed (see below).**
...
- **W5 — No explicit non-compliance/guessing detector — ADDRESSED.** *Original:* guessing was caught only indirectly by the floor/completeness gates, with no first-class validity flag (PVANC §11 anticipates one). *Fix (spec-aligned with §5.2, §8.3, §11.1):* a response-validity detector in the quality engine (`assessResponseValidity`) that flags two patterns: runs of anticipatory responses (`rapid-guessing`, §11.1) and an `invalid-response-pattern` when performance is at chance **both overall AND on the largest optotypes**. The two-part (conjunctive) test is deliberate: a guesser is at chance everywhere (flat psychometric), whereas genuine low vision is poor overall but still identifies the largest optotypes well (monotonic) — so the detector **never mislabels genuine low vision** (0 false positives across 300 simulated low-vision runs, and across all other genuine observer types). An invalid pattern suppresses the category and forces retake. *Residual (documented, fundamental):* a pure guesser who both avoids the consecutive-error floor and happens to score above the chance floors on easy and overall trials (~3–4% of guessing attempts) can still complete; such a run is information-theoretically indistinguishable from a genuine observer. The combined floor + completeness + validity stack catches the large majority; the rest carry low quality. Covered by `response-validity.test.ts` and `pvanc-observers.test.ts` (#3, plus a low-vision no-false-flag regression).

## Bottom line

The module is **scientifically sound for realistic observers** and **fails safe** on the
dangerous cases (low vision flagged, guessing rejected, inadequate devices marked
device-limited rather than mislabelled). **W2 (distance error)**, **W4 (device-limited
results)**, and **W5 (non-compliance/guessing detector)** are now fixed — all spec-compliant,
all with the raw data preserved and the limitation made explicit rather than hidden, and none
introducing a false positive on a genuine observer. **W1** remains a minor, spec-attributable
artefact at the edge of plausibility (a deliberate decision: fixing it would require changing
the spec's slope prior). **W3** (single-session calibration drift) is the only remaining open
item and needs cross-session tracking, which v1 does not yet implement.
